          ...
          <CardContent>
            ...
            {appointments.length > 0 ? (
              <p className="text-xs text-gray-500">
                Next: Dr. {appointments[0].doctorId?.firstName} {appointments[0].doctorId?.lastName} at{" "}
                {appointments[0].time}
              </p>
            ) : (
            ...
            <div className="px-4 pb-4">
              {prescriptions.map((prescription, index) => (
                <div key={index} className="py-2 border-t">
                  ...
                  <p className="text-xs text-gray-500">
                    {prescription.dosage} - {prescription.frequency} - {" (Till - "}
            ...
            <ul className="space-y-2">
              {careTeam.map((member, index) => (
                <li key={index} className="flex items-center space-x-2">
                  ...
                  <span>
                    Dr. {member.firstName} {member.lastName} - {member.specialty}
                  </span>
                </li>
              ))}
            </ul>
      ...
      <Card>
        ...
        <CardContent>
          {prescriptions.length === 0 ? (
            <p className="text-center text-gray-500 py-4">No prescriptions found</p>
          ) : (
            <div className="space-y-4">
              {prescriptions.map((prescription) => (
                <div key={prescription._id} className="p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Valid until: {new Date(prescription.tilldate).toLocaleDateString()}
                      </p>
                      <p className="text-sm text-gray-600 mt-2">
                        Prescribed by: Dr. {prescription.doctorId?.firstName} {prescription.doctorId?.lastName}
                      </p>
                      {prescription.notes && <p className="text-sm text-gray-600 mt-2">Notes: {prescription.notes}</p>}
                    </div>
