...
import { markMessagesAsRead, getOrCreateConversation, getUserById } from "../utils/messageService.js"
...
router.get("/conversations/:conversationId/files", auth, async (req, res) => {
  try {
    const { conversationId } = req.params
    const userId = req.user.id
    const userRole = req.user.role
    ...
    // Get messages with attachments for this conversation
...
router.patch("/conversations/:conversationId/read", auth, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      messagesMarkedAsRead: result.modifiedCount || 0,
    })
  } catch (error) {
...
// Enhance the download attachment endpoint to set proper headers
router.get("/attachments/:messageId", auth, async (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", message.attachment.mimetype || "application/pdf")
    res.setHeader("Content-Disposition", `attachment; filename="${message.attachment.filename}"`)

    // Send the file
    res.sendFile(path.resolve(message.attachment.path))
  } catch (error) {
